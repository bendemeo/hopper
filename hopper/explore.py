...
def viz(adata, rep = '',louvain=True, leiden=False, rerun=False, **kwargs):
    '''visualize an AnnData object using UMAP and Louvain'''

    if leiden == True and louvain == True:
        raise Exception('Both Leiden and Louvain selected. Choose one or the other.')
    ...
    if rerun or 'leiden' not in adata.obs:
        if leiden == True:
            sc.tl.leiden(adata)
